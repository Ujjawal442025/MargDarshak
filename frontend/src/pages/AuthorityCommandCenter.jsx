...
export default function AuthorityCommandCenter({ onLogout }) {
  ...
  const [surveyData, setSurveyData] = useState({
    ...
    waitMinutes: 25,
    officerNotes:
      "Suraj Pol turnstiles experiencing high volume due to group arrivals.",
  });
  ...
  const selectedDest = useMemo(() => {
    return (
      destinations.find((d) => d.site_id === selectedSiteId) || destinations[0]
    );
  ...
  const activeAlerts = useMemo(
    () =>
      getActiveAlerts(destinations).filter(
  ...
  const flowEstimate = useMemo(
    () => getFlowEstimate(selectedDest),
    [selectedDest],
  );
  ...
  const avgCapacityPct = useMemo(() => {
    ...
    const sum = destinations.reduce(
      (acc, d) => acc + (d.crowd?.current_capacity_utilization || 0),
      0,
    );
  ...
  const stateTrend = useMemo(() => {
    ...
    return {
      ...
      label:
        net > 0
          ? `${up} sites trending up`
          : net < 0
            ? `${down} sites trending down`
  ...
  const priorityQueue = useMemo(() => {
    return activeAlerts.slice(0, 3).map((alert) => {
      ...
      if (cap >= 88) action = "Immediate rerouting advised — divert inflow now";
        ...
        action = "Prepare secondary gate / processing channel";
  ...
  const handleExportReport = () => {
    ...
    const lines = [
      ...
      ...priorityQueue.map(
        (a, i) =>
          `${i + 1}. ${a.siteName} (${a.city}) — ${a.capacityPct}% capacity, ${a.crowdLevel}. Action: ${a.action}`,
      ),
      ``,
      `FULL ALERT LIST (${activeAlerts.length})`,
      ...activeAlerts.map(
        (a) =>
          `- ${a.siteName}: ${a.capacityPct}% (${a.crowdLevel}), trend ${a.trend || "n/a"}, wait ${a.estimatedWait || "n/a"}`,
      ),
    ];
  ...
  const handleExecuteInterventions = () => {
    const appliedScenario =
      whatIfScenario === "none" ? "reroute_20" : whatIfScenario;
        ...
        if (d.site_id === selectedSiteId) {
          return {
            ...
            crowd: {
              ...
              estimated_wait_time:
                newWaitMin != null
                  ? `${newWaitMin} mins (post-action, simulated)`
                  : d.crowd?.estimated_wait_time,
            },
          };
        }
        return d;
    ...
    setActionSuccessMsg(
      `Action dispatched at ${selectedDest.site_name}: "${result.label}". Projected capacity ${result.before.capacityPct}% → ${newCap}% (simulated).`,
    );
  ...
  const handleSurveySubmit = (e) => {
    ...
    setActionSuccessMsg(
      `Live Ground Telemetry for ${selectedDest.site_name} updated successfully.`,
    );
    setTimeout(() => setActionSuccessMsg(""), 5000);
  };
  ...
  const modalFilteredList = useMemo(() => {
    ...
    if (filterModalCategory === "CRITICAL") {
      return destinations.filter(
        (d) =>
          d.crowd?.current_crowd_level === "CRITICAL" ||
          d.crowd?.current_capacity_utilization >= 85,
      );
    }
    if (filterModalCategory === "HIGH") {
      return destinations.filter(
        (d) =>
          d.crowd?.current_crowd_level === "HIGH" ||
          (d.crowd?.current_capacity_utilization >= 75 &&
            d.crowd?.current_capacity_utilization < 85),
      );
    }
    if (filterModalCategory === "MODERATE") {
      return destinations.filter(
        (d) => d.crowd?.current_crowd_level === "MODERATE",
      );
    }
    ...
    <div className="min-h-screen bg-[#080C16] text-slate-100 font-sans pb-20">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500 hidden sm:block">
                Rajasthan Tourism Intelligence &amp; Crowd Management System
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2.5 text-xs">
              ...
              {searchFocused && searchQuery.trim() && (
                <div className="absolute right-0 mt-1.5 w-80 max-h-80 overflow-y-auto bg-slate-900/70 border border-slate-800 rounded-2xl shadow-xl z-40 p-2">
                  {searchResults.length === 0 ? (
                    <p className="text-[11px] text-slate-500 p-3 text-center">
            ...
            <button
              onClick={() => {
                setSurveyData({
                  newCapacityPct:
                    selectedDest.crowd?.current_capacity_utilization || 75,
      ...
      {actionSuccessMsg && (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-4">
          <div className="bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 text-xs px-4 py-3 rounded-2xl flex items-center justify-between shadow-xs animate-fade-in">
            ...
            <span className="text-[10px] uppercase font-bold text-emerald-400 bg-emerald-500/15 px-2 py-0.5 rounded-md">
              Live Updated
            </span>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 space-y-6">
        ...
        <div className="bg-gradient-to-r from-amber-500/10 via-slate-900/70 to-slate-900/70 border border-amber-500/20 rounded-3xl p-5 flex flex-col md:flex-row md:items-center gap-4">
          ...
          <div className="flex items-center gap-2 shrink-0 self-start md:self-center">
            <div className="text-center px-3 py-2 rounded-xl bg-slate-800/60 border border-slate-800">
              ...
              <strong className="text-lg font-mono text-slate-100">
                {avgCapacityPct}%
              </strong>
            </div>
            <div className="text-center px-3 py-2 rounded-xl bg-slate-800/60 border border-slate-800 flex flex-col items-center">
              ...
              <span
                className={`flex items-center gap-1 font-bold text-sm ${
                  stateTrend.direction === "up"
                    ? "text-rose-400"
                    : stateTrend.direction === "down"
                      ? "text-emerald-400"
                      : "text-slate-300"
                }`}
              >
        ...
        <div>
          <div className="flex items-center justify-between mb-2 px-1">
            <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">
              State Telemetry Summary (Click any card to inspect all matching
              destinations)
            </span>
            ...
            <div className="bg-slate-900/70 border border-blue-500/30 rounded-2xl p-4 shadow-xs">
              ...
              <div className="flex items-baseline gap-2 mt-1">
                <span className="text-2xl font-black text-blue-400 font-mono">
                  {dynamicKPIs.activeEvents}
                </span>
                <span className="text-[10px] text-slate-500">Sites</span>
              </div>
              <p className="text-[10px] text-blue-400 mt-1 truncate">
                {destinations
                  .filter((d) => d.events?.festival_today)
                  .slice(0, 2)
                  .map((d) => d.events.festival_name || d.site_name)
                  .join(", ") || "None reported today"}
              </p>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`px-2 py-0.5 rounded-full text-[10px] font-extrabold ${
                      selectedDest.crowd?.current_crowd_level === "CRITICAL"
                        ? "bg-rose-500/15 text-rose-400"
                        : "bg-orange-500/15 text-orange-400"
                    }`}
                  >
                    {selectedDest.crowd?.current_crowd_level} •{" "}
                    {selectedDest.crowd?.current_capacity_utilization}%
                  </span>
                </div>
                    ...
                    onClick={() => {
                      setSurveyData({
                        newCapacityPct:
                          ...
                          75,
                        crowdLevel:
                          selectedDest.crowd?.current_crowd_level || "HIGH",
                        waitMinutes: 20,
                        officerNotes: "Survey logged at gate terminal.",
                      });
                      setSurveyModalOpen(true);
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-1">
                <div className="p-2.5 bg-slate-800/50 rounded-xl border border-slate-800 text-center">
                  <span className="text-[9px] uppercase font-bold text-slate-500 block">
                    Estimated Visitors Now
                  </span>
                  <strong className="text-sm text-slate-100 font-mono">
                    {selectedDest.crowd?.current_visitor_count?.toLocaleString() ??
                      "—"}
                  </strong>
                </div>
                <div className="p-2.5 bg-slate-800/50 rounded-xl border border-slate-800 text-center">
                  ...
                  <strong className="text-sm text-slate-100 font-mono">
                    {selectedDest.crowd?.estimated_wait_time || "—"}
                  </strong>
                </div>
                <div className="p-2.5 bg-slate-800/50 rounded-xl border border-slate-800 text-center">
                  <span className="text-[9px] uppercase font-bold text-slate-500 block">
                  ...
                  <strong
                    className={`text-sm font-mono ${flowEstimate.available && flowEstimate.netAccumulationPerMin > 0 ? "text-rose-500" : "text-emerald-600"}`}
                  >
                    {flowEstimate.available
                      ? `${flowEstimate.netAccumulationPerMin > 0 ? "+" : ""}${flowEstimate.netAccumulationPerMin}`
                      : "N/A"}
                  </strong>
                </div>
                <div className="p-2.5 bg-slate-800/50 rounded-xl border border-slate-800 text-center">
                ...
                <div className="flex items-center justify-between border-b border-slate-800/70 pb-3">
                  <div>
                    ...
                    <h3 className="font-bold text-slate-100 text-sm">
                      Hourly Ingress Pattern (8 AM – 8 PM)
                    </h3>
                  </div>
                  <span className="text-xs font-mono font-bold text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded-md border border-amber-500/30">
                    Max:{" "}
                    {selectedDest.crowd?.total_daily_capacity?.toLocaleString()}{" "}
                    Cap
                  </span>
                </div>
              ...
              <div className="bg-slate-900/70 border border-slate-800/70 rounded-3xl p-5 shadow-xs space-y-3">
                <div className="flex items-center justify-between border-b border-slate-800/70 pb-3">
                  <div>
                    ...
                    <h3 className="font-bold text-slate-100 text-sm">
                      60-Min Inflow Pressure Forecast
                    </h3>
                  </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-slate-900/70 border border-amber-500/30 rounded-3xl p-5 shadow-xs space-y-4">
              <div className="flex items-center justify-between border-b border-slate-800/70 pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500">
                      Context: {selectedDest.site_name}
                    </p>
                  </div>
                </div>
                <span
                  className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${
                    (selectedDest.crowd?.current_capacity_utilization || 0) >=
                    75
              ...
              >
                ...
                <p className="text-slate-200 text-[11px] leading-relaxed">
                  Currently at{" "}
                  <strong>
                    {selectedDest.crowd?.current_capacity_utilization}% capacity
                  </strong>
                  , trend{" "}
                  {(selectedDest.crowd?.crowd_trend || "steady").toLowerCase()}.{" "}
                  Predicted peak today around{" "}
                  <strong>
                    {selectedDest.prediction?.predicted_peak_time || "N/A"}
                  </strong>{" "}
                  ({selectedDest.prediction?.prediction_confidence || "N/A"}{" "}
                  confidence). Estimated wait:{" "}
                  {selectedDest.crowd?.estimated_wait_time || "unavailable"}.
                </p>
              </div>
              ...
              <div className="space-y-2 text-xs text-slate-300">
                {[
                  {
                    key: "reroute_20",
                    label: "Reroute 20% of inflow to a nearby lower-crowd site",
                  },
                  {
                    key: "second_gate",
                    label: "Open a secondary entry/processing channel",
                  },
                  {
                    key: "gate_closure",
                    label: "Temporary gate closure (not recommended)",
                  },
                ].map((opt) => (
                  <label
                    key={opt.key}
                ...
                <button
                  onClick={() => {
                    const alert = activeAlerts.find(
                      (a) => a.siteId === selectedSiteId,
                    );
                    if (alert) handleDismissAlert(alert.id);
                    setActionSuccessMsg(
                      alert
                        ? `Alert for ${selectedDest.site_name} dismissed.`
                        : `${selectedDest.site_name} has no active alert to dismiss.`,
                    );
                    setTimeout(() => setActionSuccessMsg(""), 4000);
                  }}
              ...
              <div className="p-3 bg-slate-800/50 rounded-xl border border-slate-800 text-[11px] space-y-1">
                <div className="flex justify-between">
                  <span className="text-slate-500">
                    Projected capacity if approved:
                  </span>
                  <strong className="text-slate-100">
                    {whatIfResult.before.capacityPct}% →{" "}
                    {whatIfResult.after.capacityPct}%
                  </strong>
                </div>
                {whatIfResult.after.waitMin != null && (
                  <div className="flex justify-between">
                    ...
                    <strong className="text-slate-100">
                      {whatIfResult.before.waitMin ?? "—"}m →{" "}
                      {whatIfResult.after.waitMin}m
                    </strong>
                  </div>
                )}
            ...
            <div className="bg-slate-900/70 border border-slate-800/70 rounded-3xl p-5 shadow-xs space-y-4">
              <div className="flex items-center justify-between border-b border-slate-800/70 pb-3">
                <h3 className="font-bold text-slate-100 text-sm">
                  Resource Reallocation Guidance
                </h3>
              ...
              {donorSuggestion.available ? (
                <div className="space-y-2 text-xs">
                  <div className="p-3 bg-slate-800/50 rounded-xl border border-slate-800 flex items-center justify-between">
                    <div>
                      <span className="text-slate-500 block text-[10px]">
                        Target Site (needs support)
                      </span>
                      <strong className="text-slate-200 text-xs">
                        {selectedDest.site_name}
                      </strong>
                    </div>
                    <strong className="text-rose-400 text-xs bg-rose-500/10 px-2 py-0.5 rounded border border-rose-500/30">
                      {donorSuggestion.targetCapacityPct}%
                    </strong>
                  </div>
                  ...
                  <div className="p-3 bg-slate-800/50 rounded-xl border border-slate-800 flex items-center justify-between">
                    <div>
                      <span className="text-slate-500 block text-[10px]">
                        Nearest lower-pressure site (
                        {donorSuggestion.distanceKm} km)
                      </span>
                      <strong className="text-slate-200 text-xs">
                        {donorSuggestion.donorSiteName},{" "}
                        {donorSuggestion.donorCity}
                      </strong>
                    </div>
                    <strong className="text-emerald-400 text-xs bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/30">
                      {donorSuggestion.donorCapacityPct}%
                    </strong>
                  </div>
                ...
                <p className="text-[11px] text-slate-500 leading-relaxed">
                  No lower-pressure site found nearby for{" "}
                  {selectedDest.site_name}. Personnel/vehicle deployment data is
                  not yet connected to a live feed for this site.
                </p>
              )}
            </div>
              ...
              <div className="p-3 bg-slate-800/50 rounded-xl border border-slate-800 text-[11px] space-y-1.5">
                ...
                {whatIfResult.after.waitMin != null && (
                  <div className="flex justify-between">
                    ...
                    <strong
                      className={
                        whatIfResult.after.waitMin >
                        (whatIfResult.before.waitMin || 0)
                          ? "text-rose-400"
                          : "text-emerald-400"
                      }
                    >
                      {whatIfResult.after.waitMin} mins
                    </strong>
                  </div>
                )}
                <div className="flex justify-between">
                  ...
                  <strong className="text-slate-100">
                    {whatIfResult.after.visitors?.toLocaleString()}
                  </strong>
                </div>
                <p className="text-slate-500 pt-1 leading-relaxed">
                  {whatIfResult.assumption}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-xs animate-fade-in">
          <div className="bg-[#0D1526] rounded-3xl max-w-3xl w-full p-6 shadow-2xl border border-slate-800 relative max-h-[85vh] flex flex-col">
            <div className="flex items-center justify-between border-b border-slate-800/70 pb-4 mb-4 shrink-0">
              <div>
                <span className="text-[10px] uppercase font-bold text-amber-400 block">
                  Ground Intelligence Drill-down
                </span>
                <h2 className="font-serif-title text-xl font-bold text-slate-100">
                  All {filterModalCategory} Destinations (
                  {modalFilteredList.length} Found)
                </h2>
              </div>
              <button
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] text-slate-500">
                        {dest.city}, Rajasthan • {dest.category}
                      </p>
                    </div>
                  </div>
                    ...
                    <div className="text-right text-xs">
                      ...
                      <strong
                        className={`font-black ${
                          dest.crowd?.current_capacity_utilization >= 85
                            ? "text-rose-500"
                            : dest.crowd?.current_capacity_utilization >= 75
                              ? "text-orange-500"
                              : "text-slate-300"
                        }`}
                      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-xs animate-fade-in">
          <div className="bg-[#0D1526] rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-slate-800 relative">
            <div className="flex items-center justify-between border-b border-slate-800/70 pb-3 mb-4">
              <div>
                <span className="text-[10px] uppercase font-bold text-amber-400 block">
                  Ground Officer Ingress Update
                </span>
                <h3 className="font-bold text-slate-100 text-base">
                  Update Telemetry: {selectedDest.site_name}
                </h3>
              </div>
              <button
            ...
            <form onSubmit={handleSurveySubmit} className="space-y-4 text-xs">
              <div>
                <label className="font-bold text-slate-300 block mb-1">
                  Live Capacity Utilization (%): {surveyData.newCapacityPct}%
                </label>
                <input
                  type="range"
                  min="20"
            ...
            <div className="overflow-y-auto flex-1 px-5 py-4 space-y-3">
              {activeAlerts.length === 0 ? (
                <div className="text-center py-16 text-slate-500 text-xs space-y-2">
                  ...
                  <p>
                    No active alerts. All monitored sites are within normal
                    thresholds, or all alerts have been dismissed.
                  </p>
                </div>
              ) : (
